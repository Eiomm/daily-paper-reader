---
title: "World Models for Embodied Intelligence: From Plausible to Controllable to Actionable"
title_zh: 面向具身智能的世界模型：从合理到可控再到可行动
authors: "Nanjie Yao, Hao Wang, Chong Cheng, Zhikang Chen, Wenzhe Li, Jiafei Lyu, Li Shen, Peilin Zhao, Zongqing Lu, Gao Huang, Steven Hoi, Dacheng Tao, Deheng Ye"
date: 2026-09-15
pdf: "https://arxiv.org/pdf/2609.16697v1"
tags: ["query:world-models"]
score: 7.0
evidence: 从合理到可行动的具身智能世界模型综述
tldr: 世界模型在具身智能中连接感知与决策，但常以视觉保真度衡量，忽视行为提升。本文提出Plausible、Controllable、Actionable三级能力框架，并构建3x4矩阵，交叉几何、物理、动作基础与数据、奖励、策略、模型改进闭环。综述操作、导航、运动、自动驾驶及通用具身学习，梳理技术演进与评估协议。指出长时程一致性、不确定性校准、因果干预测试等挑战，推动评估从视觉逼真转向任务相关状态、干预效果与闭环行为改进。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2609.16697v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1748, \"height\": 765}, {\"url\": \"assets/figures/arxiv/2609.16697v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1700, \"height\": 1724}, {\"url\": \"assets/figures/arxiv/2609.16697v1/fig-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1736, \"height\": 884}, {\"url\": \"assets/figures/arxiv/2609.16697v1/fig-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1739, \"height\": 547}, {\"url\": \"assets/figures/arxiv/2609.16697v1/fig-005.webp\", \"caption\": \"\", \"page\": 0, \"index\": 5, \"width\": 1738, \"height\": 804}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2609.16697v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1721, \"height\": 1481}, {\"url\": \"assets/tables/arxiv/2609.16697v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1759, \"height\": 787}, {\"url\": \"assets/tables/arxiv/2609.16697v1/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1762, \"height\": 989}, {\"url\": \"assets/tables/arxiv/2609.16697v1/table-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1754, \"height\": 746}]"
motivation: 现有综述按架构、输出模态或应用领域组织，未回答哪些预测能力真正提升行为。
method: 提出Plausible、Controllable、Actionable三级能力框架，辅以3x4矩阵交叉几何、物理、动作基础与改进闭环。
result: 综述操作、导航、运动、自动驾驶与通用具身学习，梳理技术演进、能力要求、数据集、基准与评估协议。
conclusion: 评估应从视觉逼真度转向预测是否捕捉任务相关状态、反映干预效果并改进具身智能体闭环行为。
---

## 摘要
世界模型通过在具身智能中维护隐藏状态、预判后果、比较干预，并在执行偏离预期时进行适应，将感知与决策连接起来。尽管进展常以视觉保真度衡量，但其价值在于改善行为。在伸手拿杯子之前，人会预判其重量和抓握阻力，并在接触前调整手的形态。这种预判是粗略的，且很少以图像形式呈现，却能指导行动。这引出一个核心问题：哪些预测能力能够改善行为？现有综述按架构、输出模态或应用领域组织，却未明确回答这一问题。我们引入三个逐步增强的能力层级：合理模型保留任务相关的时间、几何或物理结构；可控模型还能预测干预如何改变该结构；可行动模型则将预测转化为在规划、行动、学习、评估、验证、恢复或数据选择方面可度量的收益。我们用 3×4 矩阵补充这一层级体系，该矩阵将几何、物理和动作接地与以数据、奖励、策略和模型自身为中心的改进循环交叉结合。借助这一框架，我们综述了操作、导航、运动、自动驾驶和一般具身学习，梳理技术进展，阐明能力要求，并考察数据集、基准和评估协议。我们识别出长时程一致性、不确定性校准、因果干预测试、延迟、验证与恢复以及跨具身迁移方面的挑战。这一视角将评估从视觉合理性转向预测是否捕获任务相关状态、是否反映干预效果，以及是否改善具身智能体的闭环行为。

## Abstract
World models connect perception and decision-making in embodied intelligence by maintaining hidden state, anticipating consequences, comparing interventions, and adapting when execution departs from expectations. Although progress is often measured by visual fidelity, their value lies in improving behavior. Before reaching for a cup, a person anticipates its weight and resistance to grasping, shaping the hand before contact. Such anticipation is coarse and rarely pictorial, yet it guides action. This raises a central question: which predictive capabilities improve behavior? Existing surveys, organized by architecture, output modality, or application domain, leave this question implicit. We introduce three progressively stronger capability levels: Plausible models preserve task-relevant temporal, geometric, or physical structure; Controllable models additionally predict how interventions alter that structure; and Actionable models translate predictions into measurable gains in planning, action, learning, evaluation, verification, recovery, or data selection. We complement this hierarchy with a 3 x 4 matrix crossing geometry, physics, and action grounding with improvement loops centered on data, rewards, policies, and the model itself. Using this framework, we survey manipulation, navigation, locomotion, autonomous driving, and general embodied learning, tracing technical progressions, clarifying capability requirements, and examining datasets, benchmarks, and evaluation protocols. We identify challenges in long-horizon consistency, uncertainty calibration, causal intervention testing, latency, verification and recovery, and cross-embodiment transfer. This perspective shifts evaluation from visual plausibility toward whether predictions capture task-relevant state, reflect intervention effects, and improve the closed-loop behavior of embodied agents.