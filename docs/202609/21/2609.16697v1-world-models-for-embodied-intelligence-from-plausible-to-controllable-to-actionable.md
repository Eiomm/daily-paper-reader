...
tldr: 世界模型在具身智能中连接感知与决策，但其价值不应只看视觉保真度，而应看能否提升行为。该综述提出“可信—可控—可行动”三级能力框架，并用几何、物理、动作接地与数据、奖励、策略、模型改进回路构成3×4矩阵。作者据此梳理操作、导航、运动、自动驾驶和通用具身学习，比较数据集、基准与评测协议。核心观点是评测应从视觉逼真转向任务相关状态、干预效应和闭环行为增益。
source: arxiv
...
motivation: 现有综述多按架构、输出模态或应用领域组织，未明确回答哪些预测能力真正改善具身行为。
method: 提出可信、可控、可行动三级能力层级，并以几何、物理、动作接地与数据、奖励、策略、模型四类改进回路构成3×4分析矩阵。
result: 据此系统梳理操作、导航、运动、自动驾驶与通用具身学习，追踪技术演进并比较数据集、基准和评测协议。
conclusion: 指出长时程一致性、不确定性校准、因果干预测试、延迟、验证恢复与跨具身迁移等挑战，主张以闭环行为增益衡量世界模型。
---

## 摘要
世界模型通过维护隐藏状态、预判后果、比较干预，并在执行偏离预期时进行适应，从而在具身智能中连接感知与决策。尽管进展常以视觉保真度来衡量，但其价值在于改进行为。在伸手拿杯子之前，人会预判其重量和抓握阻力，并在接触前调整手形。这种预判是粗略的，且很少以图像形式呈现，却能指导行动。这引出一个核心问题：哪些预测能力能够改进行为？现有综述通常按架构、输出模态或应用领域组织，但未明确回答这一问题。我们引入三个逐步增强的能力层级：合理模型保留任务相关的时间、几何或物理结构；可控模型还能预测干预如何改变该结构；可行动模型则将预测转化为在规划、行动、学习、评估、验证、恢复或数据选择方面的可测量收益。我们用一个 3×4 矩阵补充该层级，该矩阵将几何、物理和动作接地与以数据、奖励、策略和模型本身为中心的改进循环交叉。使用该框架，我们综述了操作、导航、运动、自动驾驶和一般具身学习，追溯技术演进，阐明能力需求，并考察数据集、基准和评估协议。我们识别出在长时程一致性、不确定性校准、因果干预测试、延迟、验证与恢复以及跨具身迁移方面的挑战。这一视角将评估从视觉合理性转向预测是否捕获任务相关状态、反映干预效果，并改善具身智能体的闭环行为。
